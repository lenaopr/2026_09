...
ActiveUsers AS (
    -- Attended booking during the tracking period.
    SELECT DISTINCT u.SSOUserId
    ...
    -- Successfully purchased a voucher, even if not redeemed yet.
    SELECT DISTINCT vo.SSOUserId
    FROM dbo.VoucherOrder vo WITH (NOLOCK)
    INNER JOIN dbo.PaymentTransaction pt WITH (NOLOCK)
        ON pt.PaymentTransactionId = vo.PaymentTransactionId
    WHERE pt.CommodityType = 1
        AND pt.Status = 10
        AND pt.PaymentTime >= @StartDate
        AND pt.PaymentTime < @EndDate
        AND vo.SSOUserId IS NOT NULL
...
SELECT
    ...
    (SELECT COUNT(*) FROM ActiveUsers) AS ActiveUsers;